...
def process_dir(root_dir, subdir, template):
    ...
    if(template and template.marker_path):
        ...
        # Ignore other files if 
        if (len(args['img_file']) > 0):
            omr_files = [args['img_file']]
...
def process_files(omr_files, template, args, out):
    ...
    for filepath in omr_files:
        ...
        jsonFilePath = out.paths.resultDir+filename+".json"
        print("Writing data to: "+jsonFilePath)
...
argparser.add_argument("-f", "--filename", default='', required=False,
                       dest='img_file', help="Specify an single image file for OMR.")
